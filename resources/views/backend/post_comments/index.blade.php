                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        @forelse($comments as $comment)
                            <tr>
                                <td><img src="{{ get_gravatar($comment->email) }}" class="img-circle"></td>
                                <td><a href="{!! $comment->url != '' ? : 'javascript:void(0)' !!}" target="_blank">{{ $comment->name }}</a> {{ $comment->user_id != null ? '(Member)' : '' }}</td>
                                <td>
                                    {!! $comment->comment !!}
                                ...
                                <td>
                                    <div class="btn-group">
                                        ...
                                        <a href="javascript:void(0)" onclick="if (confirm('Are you sure to delete this comment?')) { event.preventDefault(); document.getElementById('comment-delete-{{ $comment->id }}').submit();} else {return false;}" class="btn btn-danger"><i class="fa fa-trash"></i></a>
